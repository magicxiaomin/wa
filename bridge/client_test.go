...
func TestRemoteCommandContactsUsesGetContacts(t *testing.T) {
	events := newEventRecorder()
	fake := newFakeWAAdapter()
	c := newStartedConnectedTestClient(t, events, fake)

	response := c.handleRemoteCommand(remoteCommandFrame{
		Type:      "contacts",
		RequestID: "remote-contacts-1",
	})
	if !response.OK {
		t.Fatalf("remote contacts response = %#v", response)
	}
	if response.RequestID != "remote-contacts-1" {
		t.Fatalf("request id = %q", response.RequestID)
	}
	if !strings.Contains(response.ResultJSON, `"jid":"15551234567@s.whatsapp.net"`) {
		t.Fatalf("contacts result missing contact: %s", response.ResultJSON)
	}
}

func TestRemoteCommandSendUsesSendTextMultiAndWrapperLimit(t *testing.T) {
	restore := overrideMultiSendTestTiming()
	defer restore()
	...
	response := c.handleRemoteCommand(remoteCommandFrame{
		Type:        "send",
		RequestID:   "remote-send-1",
		ToJids:      []string{"15550000001@s.whatsapp.net", "15550000002@s.whatsapp.net"},
		Text:        "hello remote",
		ClientMsgID: "remote-client-1",
	})
	if !response.OK {
		t.Fatalf("remote send response = %#v", response)
	}
	if fake.sendCalls != 2 {
		t.Fatalf("send calls = %d, want 2", fake.sendCalls)
	}
	if strings.Contains(response.ResultJSON, "hello remote") {
		t.Fatalf("remote send result leaked message text: %s", response.ResultJSON)
	}

	blocked := c.handleRemoteCommand(remoteCommandFrame{
		Type:      "send",
		RequestID: "remote-send-2",
		ToJids: []string{
			"15550000001@s.whatsapp.net",
			"15550000002@s.whatsapp.net",
			"15550000003@s.whatsapp.net",
		...
		Text:        "blocked",
		ClientMsgID: "remote-client-2",
	})
	if blocked.OK || blocked.ErrorCode != "too_many_recipients" {
		t.Fatalf("over-limit response = %#v", blocked)
	}
	if fake.sendCalls != 2 {
		t.Fatalf("over-limit remote send reached adapter: calls=%d", fake.sendCalls)
	}
}

func TestStartRemoteRelayValidatesTokenAndWSS(t *testing.T) {
	events := newEventRecorder()
	fake := newFakeWAAdapter()
	c := newStartedConnectedTestClient(t, events, fake)

	if err := c.StartRemoteRelay("wss://relay.example/ws", "short"); err == nil || !strings.Contains(err.Error(), "at least 32") {
		t.Fatalf("short token error = %v", err)
	}
	if err := c.StartRemoteRelay("ws://relay.example/ws", strings.Repeat("a", 32)); err == nil || !strings.Contains(err.Error(), "wss") {
		t.Fatalf("non-wss URL error = %v", err)
	}
	if err := c.StartRemoteRelay("wss://relay.example/not-ws", strings.Repeat("a", 32)); err == nil || !strings.Contains(err.Error(), "/ws") {
