...
class BridgeForegroundService : Service() {
    ...
    private val binder = object : IBridgeService.Stub() {
        ...
        override fun getSelfIdentity(): String {
            return callWithResult("getSelfIdentity", "{}") { ensureClient("WA-Android").selfIdentity }
        }
        ...
        override fun getUserInfo(jidsJson: String): String {
            return callWithResult("getUserInfo", "[]") { ensureClient("WA-Android").getUserInfo(jidsJson) }
        }

        override fun getProfilePictureInfo(jid: String): String {
            return callWithResult("getProfilePictureInfo", "{}") { ensureClient("WA-Android").getProfilePictureInfo(jid) }
        }
        ...
        override fun markRead(chatJid: String, messageIdsJson: String, senderJid: String) {
            call("markRead") { ensureClient("WA-Android").markRead(chatJid, messageIdsJson, senderJid) }
        }

        override fun sendPresence(state: String) {
            call("sendPresence") { ensureClient("WA-Android").sendPresence(state) }
        }

        override fun subscribePresence(jid: String) {
            call("subscribePresence") { ensureClient("WA-Android").subscribePresence(jid) }
        }

        override fun exportTrace(path: String) {
            call("exportTrace") { ensureClient("WA-Android").exportTrace(privateFilesPath(path)) }
        }

        override fun exportSessionDebug(path: String) {
            call("exportSessionDebug") { ensureClient("WA-Android").exportSessionDebug(privateFilesPath(path)) }
        }
    ...
    private fun privateFilesPath(path: String): String {
        val root = filesDir.canonicalFile
        val target = File(path).canonicalFile
        if (target.path != root.path && !target.path.startsWith(root.path + File.separator)) {
            throw IllegalArgumentException("path must be under app private filesDir")
        }
        return target.path
    }
